...
(function($) {
	...
	function initTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0; i < treeNodes.length; i++) {
			...
			node.level = level;
			/*node.tId = setting.treeObjId + (++zTreeId);*/
			//此处修改方便大家自己定义id
			node.tId = node[setting.treeNodeKey];
	...
	function showTree(setting, treeNode) {
		...
		var html = "<li id='" + treeNode.tId + "' class='"+treeNode.tId+"'>'" + "<button type=\"button\" id='" + treeNode.tId + IDMark_Switch + "' title='' onfocus='this.blur();'></button>" + "<a id='" + treeNode.tId + IDMark_A + "'class='"+setting.iconSkin+"' onclick=\"" + (treeNode.click || '') + "\" ><button type=\"button\" id='" + treeNode.tId + IDMark_Icon + "' title='' onfocus='this.blur();'></button><span id='" + treeNode.tId + IDMark_Span + "'></span></a>" + "<ul id='" + treeNode.tId + IDMark_Ul + "'></ul>" + "</li>";
		p.append(html);
	...
	//设置PId
	function setNodePID(setting, treeNode) {
		var aObj = $("#" + treeNode.tId);
		if (treeNode.url && !setting.editable) aObj.attr("data-pid", treeNode.pId);
		else aObj.removeAttr("data-pid");
	}
/*	//设置URL
	...
	function setNodeUrl(setting, treeNode) {
		var aObj = $("#" + treeNode.tId);
		if (treeNode.url && !setting.editable) aObj.attr("url", treeNode.url);
		else aObj.removeAttr("href");
	}
	//设置Line、Ico等css属性
